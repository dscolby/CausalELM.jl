...
## Next Steps
Like other neural networks, one challenge with ELMs is deciding on the correct number of neurons to use. Huang et al. (2006) use an approach called an Incremental Extreme Learning Machine whereby they add neurons one by one until a desired error is achieved or the maximum number of neruons are added. I believe such an approach will lead to a network whose benefits do not outweigh its training time. On the other hand, newer work proposes to use nature-inspired heuristic optimizations. These seem overcomplicated to me, especially for a causal inference setting. Rather, I will eventually implement an approach using cross validation whereby we keep track of the average loss and stop adding nodes when the loss of the current iteration converges to the average loss or we have added the maximum nuber of nodes. This is similar to the elbow method for choosing the number of clusters when using k-means clustering: we stop adding neurons when the decrease in loss levels off, which should be at the elbow.
